    ...
    class Build : NukeBuild
    {
                                                             ...
                                                             .Executes(() =>
                                                                       {
                                                                           ...
                                                                           {
                                                                               Log.Warning("Could not set Calamari version in Octopus Server " + 
                                                                                           "project {ServerProjectFile} to {NugetVersion} as could not find project file",
                                                                                           serverProjectFile, NugetVersion.Value);
                                                                           }
                                                                       });
        ...
        void SignAndTimestampBinaries(string outputDirectory)
        {
                ...
                AzureKeyVaultCertificateName.IsNullOrEmpty())
            {
                ...
                if (!OperatingSystem.IsWindows())
                    throw new InvalidOperationException("Non-windows builds must either leave binaries unsigned or sign using the AzureSignTool");
                                   ...
                                   string displayUrl = "")
        {
            ...
            Log.Information("Signing {FilesCount} files using certificate at '{CertificatePath}'...", files.Count, certificatePath);
        ...
        void TimeStampFiles(ICollection<string> files)
        {
            ...
            foreach (var url in TimestampUrls)
            {
                ...
                {
                    Log.Error(ex, "Failed to timestamp files using {Url}. Maybe we can try another timestamp service...", url);
                }
            }
