...
public class StackEventLogger(ILog log)
{
    string lastMessage;
    readonly HashSet<string> warnings = new(StringComparer.OrdinalIgnoreCase);

    /// <summary>
    /// Display a warning message to the user (without duplicates)
    /// </summary>
        ...
        bool missingIsFailure)
    {
        ...
        if (expectSuccess && !isSuccess && isStackType)
        {
            ...
            {
                // Ignore. This just means we won't display any of the status reasons
            }
