...
sealed class EcsDeployTemplate
{
        ...
        Cfn.Value<string> awsRegionRef) => new()
    {
        ...
        Essential = c.Essential.ConvertedOrDefault(s => bool.TryParse(s, out var result) && result),
        DisableNetworking = c.NetworkSettings.DisableNetworking.ConvertedOrDefault(s => bool.TryParse(s, out var result) && result),
        WorkingDirectory = string.IsNullOrEmpty(c.WorkingDirectory) ? null : c.WorkingDirectory,
        Memory = c.MemoryLimitHard.ConvertedOrDefault<int?>(s => int.TryParse(s, NumberStyles.Integer, CultureInfo.InvariantCulture, out var result) ? result : null),
        MemoryReservation = c.MemoryLimitSoft.ConvertedOrDefault<int?>(s => int.TryParse(s, NumberStyles.Integer, CultureInfo.InvariantCulture, out var result) ? result : null),
        Cpu = c.Cpus.ConvertedOrDefault<int?>(s => int.TryParse(s, NumberStyles.Integer, CultureInfo.InvariantCulture, out var result) ? result : null),
        User = string.IsNullOrEmpty(c.User) ? null : c.User,
        StartTimeout = c.StartTimeout.ConvertedOrDefault<int?>(s => int.TryParse(s, NumberStyles.Integer, CultureInfo.InvariantCulture, out var result) ? result : null),
        StopTimeout = c.StopTimeout.ConvertedOrDefault<int?>(s => int.TryParse(s, NumberStyles.Integer, CultureInfo.InvariantCulture, out var result) ? result : null),
        ...
        ReadonlyRootFilesystem = c.ContainerStorage.ReadOnlyRootFileSystem.ConvertedOrDefault(s => bool.TryParse(s, out var result) && result),
