...
public partial class LogAwsUserInfoConvention(AwsEnvironmentGeneration awsEnvironmentGeneration) : IInstallConvention
{
    ...
    async Task InstallAsync(RunningDeployment deployment)
    {
        ...
        IAmazonIdentityManagementService IdentityManagementClientFactory() => ClientHelpers.CreateIdentityManagementServiceClient(awsEnvironmentGeneration);

        IAmazonSecurityTokenService StsClientFactory() => ClientHelpers.CreateSecurityTokenServiceClient(awsEnvironmentGeneration);
    }

    static async Task WriteUseInfo(Func<IAmazonIdentityManagementService> clientFactory)
    {
        try
        {
    ...
    static async Task WriteRoleInfo(Func<IAmazonSecurityTokenService> clientFactory)
    {
        try
        {
