...
public class DeployAwsCloudFormationConvention : CloudFormationInstallationConventionBase
{
    ...
    public DeployAwsCloudFormationConvention(
        ...
        ILog log,
        TimeSpan? waitTimeout = null) : base(stackEventLogger, log)
    {
    ...
    async Task DeployCloudFormation(RunningDeployment deployment, StackArn stack, ICloudFormationRequestBuilder template)
    {
        Guard.NotNull(deployment, "deployment can not be null");

        var deploymentStartTime = DateTime.Now;

        await clientFactory.WaitForStackToComplete(PollPeriod(deployment), stack, LogAndThrowRollbacks(clientFactory, stack, false, filter: FilterStackEventsSince(deploymentStartTime)), timeout: waitTimeout);
    ...
    async Task DeployStack(RunningDeployment deployment, StackArn stack, ICloudFormationRequestBuilder template, DateTime deploymentStartTime)
    {
        Guard.NotNull(deployment, "deployment can not be null");
        ...
        var stackId = await template.Inputs
                                    ...
                                    .Map(async parameters => await StackExists(stack, StackStatus.DoesNotExist) != StackStatus.DoesNotExist
                                             ...
                                             : await CreateCloudFormation(template));

        if (waitForComplete)
        {
            await clientFactory.WaitForStackToComplete(PollPeriod(deployment), stack, LogAndThrowRollbacks(clientFactory, stack, filter: FilterStackEventsSince(deploymentStartTime)), timeout: waitTimeout);
        }
    ...
    /// </summary>
    /// <param name="stack">The stack name</param>
    /// <param name="predicate">The optional predicate used to filter events</param>
    /// <returns>The stack event</returns>
    Task<Maybe<StackEvent>> StackEvent(StackArn stack, Func<StackEvent, bool> predicate = null)
    {
    ...
    Task<string> CreateCloudFormation(ICloudFormationRequestBuilder template)
    {
    ...
    /// Deletes the stack
    /// </summary>
    Task DeleteCloudFormation(StackArn stack)
    {
        return WithAmazonServiceExceptionHandling(async () =>
                                                  {
        ...
        ICloudFormationRequestBuilder template)
    {
        ...
        catch (AmazonCloudFormationException ex)
        {
            ...
            await clientFactory.WaitForStackToComplete(PollPeriod(deployment), stack, LogAndThrowRollbacks(clientFactory, stack, false, filter: FilterStackEventsSince(deploymentStartTime)), timeout: waitTimeout);
            return await CreateCloudFormation(template);
        }
    ...
    /// <summary>
    /// Check whether the stack must be deleted to recover.
    /// </summary>
