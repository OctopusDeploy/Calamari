...
function SafelyLog-EnvironmentVars
{
    Try
    {
        ...
        $osBitVersion = If ([System.Environment]::Is64BitOperatingSystem)
        {
            "x64"
        }
        Else
...
function Report-KubernetesManifest
{
    [CmdletBinding()]
    param (
        [Parameter(ValueFromPipeline = $true)][string] $manifest,
        [string] $namespace
    )
    ...
    for ($i = 0; $i -lt $manifests.Count; $i++) {
        ...
        $convertedManifest = Convert-ServiceMessageValue($current)
        $message = "##octopus[k8s-manifest-applied manifest='$convertedManifest'"

        if (![string]::IsNullOrWhiteSpace($namespace))
        {
            $convertedNamespace = Convert-ServiceMessageValue($namespace)
            $message = "$message ns='$convertedNamespace'"
        }
...
function Report-KubernetesManifestFile
{
    [CmdletBinding()]
    param (
