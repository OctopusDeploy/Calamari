...
namespace Calamari.Integration.FileSystem
{
    ...
    public class FreeSpaceChecker : IFreeSpaceChecker
    {
        ...
        public void EnsureDiskHasEnoughFreeSpace(string directoryPath)
        {
            ...
            if (variables.GetFlag("OctopusSkipFreeDiskSpaceCheck"))
            {
                Log.Verbose($"OctopusSkipFreeDiskSpaceCheck is enabled. The check to ensure that the drive containing the directory '{directoryPath}' on machine '{Environment.MachineName}' has enough free space will be skipped.");
                return;
            }

            ulong requiredSpaceInBytes = 500L * 1024 * 1024;
            var freeSpaceOverrideInMegaBytes = variables.GetInt32("OctopusFreeDiskSpaceOverrideInMegaBytes");

            if (freeSpaceOverrideInMegaBytes.HasValue)
            {
                requiredSpaceInBytes = (ulong) freeSpaceOverrideInMegaBytes * 1024 * 1024;
                Log.Verbose($"OctopusFreeDiskSpaceOverrideInMegaBytes has been specified. We will check and ensure that the drive containing the directory '{directoryPath}' on machine '{Environment.MachineName}' has {((ulong) requiredSpaceInBytes).ToFileSizeString()} free disk space.");
            }
