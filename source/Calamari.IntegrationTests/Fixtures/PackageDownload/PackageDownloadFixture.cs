...
namespace Calamari.IntegrationTests.Fixtures.PackageDownload
{
    [TestFixture]
    public class PackageDownloadFixture : CalamariFixture
    {
        ...
        public void ShouldDownloadPackage()
        {
            ...
            result.AssertOutput("Downloaded package will be stored in: '{0}'", PackageDownloadFolder(PublicFeed));
            ...
            AssertStagePackageOutputVariableSet(result, PackageFile(PublicFeed));
        ...
        public void ShouldUsePackageFromCache()
        {
            ...
            result.AssertOutput("Package was found in cache. No need to download. Using file: '{0}", PackageFile(PublicFeed));
            AssertPackageHashMatchesExpected(result, ExpectedPackageHash);
            AssertPackageSizeMatchesExpected(result, ExpectedPackageSize);
            AssertStagePackageOutputVariableSet(result, PackageFile(PublicFeed));
        }
        ...
        public void PrivateNuGetFeedShouldDownloadPackage()
        {
            ...
            result.AssertOutput("Downloaded package will be stored in: '{0}'", PackageDownloadFolder(AuthFeed));
            result.AssertOutput("Found package {0} version {1}", AuthFeed.PackageId, AuthFeed.Version);
            AssertPackageHashMatchesExpected(result, ExpectedPackageHash);
            AssertPackageSizeMatchesExpected(result, ExpectedPackageSize);
            AssertStagePackageOutputVariableSet(result, PackageFile(AuthFeed));
        ...
        public void PrivateNuGetFeedShouldUsePackageFromCache()
        {
            ...
            result.AssertOutput("Package was found in cache. No need to download. Using file: '{0}", PackageFile(AuthFeed));
            AssertPackageHashMatchesExpected(result, ExpectedPackageHash);
            AssertPackageSizeMatchesExpected(result, ExpectedPackageSize);
            AssertStagePackageOutputVariableSet(result, PackageFile(AuthFeed));
        }
        ...
        public void FileShareFeedShouldDownloadPackage()
        {
            using (var acmeWeb = CreateSamplePackage())
            {
                ...
                result.AssertOutput("Downloaded package will be stored in: '{0}'", PackageDownloadFolder(FileShare));
                ...
                AssertStagePackageOutputVariableSet(result, PackageFile(FileShare));
        ...
        public void FileShareFeedShouldUsePackageFromCache()
        {
            using (var acmeWeb = CreateSamplePackage())
            {
                ...
                result.AssertOutput("Package was found in cache. No need to download. Using file: '{0}", PackageFile(FileShare));
                AssertPackageHashMatchesExpected(result, acmeWeb.Hash);
                AssertPackageSizeMatchesExpected(result, acmeWeb.Size);
                AssertStagePackageOutputVariableSet(result, PackageFile(FileShare));
            }
        ...
        string PackageDownloadFolder(Feed feed)
        {
            return Path.Combine(this.DownloadPath, feed.Id);
        }

        string PackageFile(Feed feed)
        {
            return Path.Combine(PackageDownloadFolder(feed), feed.PackageId + '.' + feed.Version);
        }
    }
}
