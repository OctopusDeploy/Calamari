...
using Calamari.Utilities.Retry;
using FileSizeExtensions = Calamari.Integration.FileSystem.FileSizeExtensions;
        ...
        public void EnsureDiskHasEnoughFreeSpace(string directoryPath, long requiredSpaceInBytes)
        {
            ...
            if (totalNumberOfFreeBytes < required)
            {
                throw new IOException(
                    $"The drive containing the directory '{directoryPath}' on machine '{Environment.MachineName}' does not have enough free disk space available for this operation to proceed. The disk only has {FileSizeExtensions.ToFileSizeString(totalNumberOfFreeBytes)} available; please free up at least {FileSizeExtensions.ToFileSizeString(required)}.");
            }
        }
