...
{
    // Covers the Bicep template-source resolution and the compile -> substitute -> deploy wiring of
    // DeployBicepTemplateBehaviour via a mocked IBicepCompiler (the az-cli step) + IAzureResourceGroupOperator.
    // These scenarios previously only ran against real Azure (and required the az cli) in
    // DeployAzureBicepTemplateCommandFixture, which now keeps a single Package smoke test.
    [TestFixture]
    public class DeployBicepTemplateBehaviourUnitTestFixture
    {
        ...
        public void SetUp()
        {
            bicepCompiler = Substitute.For<IBicepCompiler>();
