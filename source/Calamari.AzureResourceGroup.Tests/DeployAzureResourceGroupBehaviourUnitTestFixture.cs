...
{
    // Covers the template-source resolution, parameter normalisation and deployment mode/name logic of
    // DeployAzureResourceGroupBehaviour via a mocked IAzureResourceGroupOperator + ITemplateService.
    // These scenarios previously only ran against real Azure in AzureResourceGroupActionHandlerFixture,
    // which now keeps a single Package smoke test.
    [TestFixture]
    public class DeployAzureResourceGroupBehaviourUnitTestFixture
    {
        ...
        {
            // GitRepository and Package take the identical "files in package or repository" branch - this is
            // why the cloud fixture does not need a separate Git deploy test.
            templateService.GetSubstitutedTemplateContent("template.json", true, Arg.Any<IVariables>()).Returns("TEMPLATE");
            templateService.GetSubstitutedTemplateContent("params.json", true, Arg.Any<IVariables>()).Returns("PARAMS");
        ...
        public async Task DeploymentName_DerivedFromStepNameWhenNotProvided()
        {
            ...
            // FromStepName sanitises the step name and appends a random GUID suffix, so assert the prefix.
            await resourceGroupOperator.Received(1).Deploy(Arg.Any<IAzureAccount>(), SubscriptionId, ResourceGroupName,
