...
{
    // Single real-cloud smoke test: confirms a Bicep deploy round-trips against real Azure - this is also the
    // only test exercising the real `az bicep build` compile step (hence the Windows/az-cli requirements).
    // Template-source resolution (Package/GitRepository/Inline) and the compile -> substitute -> deploy wiring
    // is covered without Azure or the az cli in DeployBicepTemplateBehaviourUnitTestFixture via a mocked
    // IBicepCompiler + IAzureResourceGroupOperator.
    [TestFixture]
    [WindowsTest] // NOTE: We should look at having the Azure CLI installed on Linux boxes so that these steps can be tested there.
