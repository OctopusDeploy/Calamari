...
{
    // The two nested fixtures cover two distinct Azure deployment targets - a Web App and a (Linux) Function
    // App - rather than two operating systems. Calamari runs the same zip-deploy code path for both; the split
    // exists because the Function App is a different resource type (function runtime, run-from-package), not
    // because the OS differs.
