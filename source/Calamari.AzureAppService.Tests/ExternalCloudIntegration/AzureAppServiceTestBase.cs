...
{
    /// <summary>
    /// Shared base for fixtures that talk to real Azure. Authenticates with the test service principal and
    /// creates the <see cref="ArmClient" /> in a OneTimeSetUp that runs before any derived OneTimeSetUp, so
    /// subclasses can create or look up their resources against the established connection. Carries the
