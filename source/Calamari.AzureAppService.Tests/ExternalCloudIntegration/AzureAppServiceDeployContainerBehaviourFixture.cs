...
{
    /// <summary>
    /// Real-cloud smoke tests confirming a container deployment round-trips against a real Linux app
    /// service, for both the web app itself and a deployment slot (auth + slot-vs-site ARM config/app-settings
    /// GET and PATCH). A single OS is sufficient here: the Windows/Linux FxVersion branch, along with the
    /// image/registry and slot-targeting logic, is covered without Azure in
    /// AzureAppServiceContainerDeployBehaviourUnitTestFixture via a mocked IAzureAppServiceContainerConfigurer.
    /// </summary>
    public class AzureAppServiceDeployContainerBehaviourFixture
    {
        ...
        public class WhenUsingALinuxAzureAppService : AzureAppServiceWithProvisionedResourcesTestBase
        {
            ...
            public async Task AzureLinuxContainerDeploy()
            {
                ...
                await new AzureAppServiceContainerDeployBehaviour(new InMemoryLog(), new AzureAppServiceContainerConfigurer()).Execute(runningContext);
