...
using Calamari.FullFrameworkTools.Tests.WindowsCertStore;
...
namespace Calamari.Tests.Fixtures.Certificates
{
    ...
    public class WindowsX509CertificateStoreFixture
    {
        ...
        public void CanImportCertificate(string sampleCertificateId, StoreLocation storeLocation, string storeName)
        {
            ...
            new WindowsX509CertificateStore(new NullLog()).ImportCertificateToStore(Convert.FromBase64String(sampleCertificate.Base64Bytes()), sampleCertificate.Password,
        ...
        public void SafeForConcurrentOperations()
        {
            ...
            try
            {
                using (var cts = new CancellationTokenSource(maxTimeAllowedForTest))
                {
                    ...
                    new WindowsX509CertificateStore(new NullLog()).ImportCertificateToStore(
                    ...
                    var threads =
                        CreateThreads(numThreads, "ImportCertificateToStore", () =>
                            ...
                            {
                                new WindowsX509CertificateStore(new NullLog()).AddPrivateKeyAccessRules(
                            ...
                            {
                                var unused = new WindowsX509CertificateStore(new NullLog()).GetPrivateKeySecurity(
                                    sampleCertificate.Thumbprint, StoreLocation.LocalMachine, "My");
                            })).ToArray();
            ...
            StoreLocation storeLocation, string storeName)
        {
            ...
            new WindowsX509CertificateStore(new NullLog()).AddPrivateKeyAccessRules(sampleCertificate.Thumbprint, storeLocation, storeName,
                new List<PrivateKeyAccessRule>
                {
                    new PrivateKeyAccessRule("BUILTIN\\Users", PrivateKeyAccess.FullControl)
                });
            ...
            var privateKeySecurity = new WindowsX509CertificateStore(new NullLog()).GetPrivateKeySecurity(sampleCertificate.Thumbprint,
        ...
        {
            new WindowsX509CertificateStore(new NullLog()).RemoveCertificateFromStore(rootAuthorityThumbprint, StoreLocation.LocalMachine, rootAuthorityStore.Name);

            if (!string.IsNullOrEmpty(intermediateAuthorityThumbprint))
                new WindowsX509CertificateStore(new NullLog()).RemoveCertificateFromStore(intermediateAuthorityThumbprint, StoreLocation.LocalMachine, intermediateAuthorityStore.Name);
        }
