...
using AzureAppServiceProgram = Calamari.AzureAppService.Program;
...
namespace Calamari.ExternalTools.Tests.CloudIntegration.AzureAppService
{
    public class AppServiceBehaviorFixture
    {
        ...
        public class WhenUsingAWindowsDotNetAppService : AppServiceIntegrationTest
        {
            ...
            public async Task CanDeployWebAppZip()
            {
                ...
                await CommandTestBuilder.CreateAsync<DeployAzureAppServiceCommand, AzureAppServiceProgram>()
                                        .WithArrange(context =>
                                                     {
            ...
            public async Task CanDeployJarPackage()
            {
                ...
                var commandResult = await CommandTestBuilder.CreateAsync<DeployAzureAppServiceCommand, AzureAppServiceProgram>()
                                                            .WithArrange(context =>
                                                                         {
                                                                             context.WithPackage(packageinfo.packagePath, packageinfo.packageName, packageinfo.packageVersion);
                                                                             AddVariables(context);
            ...
            public async Task DeployToTwoTargetsInParallel_Succeeds()
            {
                ...
                {
                    // Act
                    var deployment = await CommandTestBuilder.CreateAsync<DeployAzureAppServiceCommand, AzureAppServiceProgram>()
                                                             .WithArrange(context =>
                                                                          {
