...
namespace Calamari.ConsolidateCalamariPackages.Tests
{
    ...
    public class ConsolidationVerificationTests
    {
        ...
        static readonly Dictionary<string, PackagePropertiesToTest>  PackagesWithDetails = new  Dictionary<string, PackagePropertiesToTest>
        {
            ...
            { "Calamari.AzureWebApp", new PackagePropertiesToTest(new[] { "netfx", "win-x64" }, false) },
            { "Calamari.Terraform", new PackagePropertiesToTest(isWindows ? AllArchitectures : NetCoreArchitectures, false)}
        };

        static bool PackageSupported(string packageId)
        {
            if (isWindows)
            {
            ...
            return MigratedCalamariFlavours.CrossPlatformFlavours.Contains(packageId) || packageId == "Calamari" || packageId == "Calamari.Cloud";
        }

        static IEnumerable<string> ExpectedPackages()
        {
            return PackagesWithDetails.Select(kvp => kvp.Key).Where(PackageSupported);
        }
        
        static IEnumerable<TestCaseData> ExpectedPackageArchitectureMappings =>
            PackagesWithDetails.Where(kvp => PackageSupported(kvp.Key)).
                                Select(kvp => new TestCaseData(kvp.Key, kvp.Value.Architectures)
                                           .SetName($"Package_{kvp.Key}_HasExpectedArchitectures"));

        static IEnumerable<TestCaseData> ExpectedPackageNugetStatus => PackagesWithDetails
                                                                       .Where(kvp => PackageSupported(kvp.Key))
                                                                       .Select(kvp => new TestCaseData(kvp.Key, kvp.Value.IsNupkg).SetName($"Package {kvp.Key} Has Expected Nuget PaackageFlag"));
        ...
        public void SetUp()
        {
            ...
            isWindows = isWindowsEnvValue == null ? RuntimeInformation.IsOSPlatform(OSPlatform.Windows) : bool.Parse(isWindowsEnvValue);
