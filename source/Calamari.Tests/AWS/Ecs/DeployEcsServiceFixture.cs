...
namespace Calamari.Tests.AWS.Ecs
{
    ...
    public class DeployEcsServiceFixture
    {
        ...
        // A hand-written template that stands in for what JT's server-side mapper will emit:
        // task definition + service (FARGATE, desiredCount 0 so no image pull required) +
        ...
        async Task<IVariables> CreateVariables(string serviceName, string cfStackName)
        {
        ...
        async Task DeleteStack(string stackName)
        {
            var credentials = await GetCredentials();
            var config = new AmazonCloudFormationConfig { RegionEndpoint = RegionEndpoint.GetBySystemName(Region) };
